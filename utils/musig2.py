...
from .taproot_sighash import SIGHASH_DEFAULT, TaprootSignatureHash
...
from ledger_bitcoin.key import G, ExtendedKey, bytes_to_point, point_add, point_mul, point_to_bytes
from ledger_bitcoin.wallet import WalletPolicy
from ledger_bitcoin import MusigPubNonce, MusigPartialSignature
...
def process_placeholder(
    ...
    if tapleaf_desc is None:
        ...
        if desc_tmpl.tree is not None:
            t += desc_tmpl.get_taptree_hash(wallet_policy.keys_info,
...
class LedgerMusig2Cosigner(PsbtMusig2Cosigner):
    """
    Implements a PsbtMusig2Cosigner that uses a Ledger device to sign a PSBT for a wallet policy that contains
    a musig() key expression.
    """
